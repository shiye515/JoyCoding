...
定义手柄发现、设备与按键选择、输入状态可视化及映射设置占位的基线行为。
...
应用 SHALL 在左栏的垂直列表显示当前设备通过标准 extended gamepad profile 暴露的按钮，并 SHALL 在无法使用该 profile 时以 physical input profile 提供合理回退。每个条目 SHALL 具有稳定标识、可读名称和选中状态。
...
- **THEN** 该条目显示为当前选中按键，且右栏更新为该按键的详情
...
右栏 SHALL 为当前选中按键显示映射设置占位；在真正映射功能实现前，映射目标 SHALL 使用当前按键名称作为只读占位值，且 MUST NOT 发送键盘事件或写入映射配置。

#### Scenario: 显示选中按键的映射占位
- **WHEN** 用户选择一个按键
- **THEN** 右栏显示该按键名称和以相同名称填充的“映射到”占位值

#### Scenario: 未选择按键
- **WHEN** 当前没有可用设备或可用按键
- **THEN** 右栏显示引导性空状态而不显示映射字段
