...
首次发现设备时自动选中列表首项；用户选择后保持该选择；非选中设备断开不改变选择；选中设备断开时选择剩余列表首项；没有设备时选择为空。设备列表使用水平 `ScrollView`，底部根据选择展示按钮列表或空状态。
...
3. 替换默认 `ContentView`，完成上下分区、选择和高亮界面。
